...
const Form: React.FC<FormProps> = ({ onFormSubmit, formFields }) => {
  ...
  const isTour = localStorage.getItem('SOWP_BUILDER_TOUR_DISABLE');
  ...
  const handleJoyrideCallback = (data: any) => {
    ...
    if ([STATUS.FINISHED, STATUS.SKIPPED].includes(status) || action === 'close') {
      localStorage.setItem('SOWP_BUILDER_TOUR_DISABLE', 'Yes');
  ...
  const handleNextSection = () => {
    if (areRequiredFieldsFilled()) {
      ...
      if (formData.marriageType === 'love' && sections[nextSection].title === 'Relationship (Arrange)') {
        nextSection += 1;
      } else if (formData.marriageType === 'arranged' && sections[nextSection].title === 'Relationship (Love)') {
        nextSection += 1;
      } else if (formData.spousePermitStatus === 'work' && sections[nextSection].title === 'On Study Permit') {
        nextSection += 1;
      } else if (formData.spousePermitStatus === 'study' && sections[nextSection].title === 'On Work Permit') {
        nextSection += 1;
      }
  ...
  const handlePreviousSection = () => {
    ...
    if (formData.marriageType === 'love' && sections[prevSection].title === 'Relationship (Arrange)') {
      prevSection -= 1;
    } else if (formData.marriageType === 'arranged' && sections[prevSection].title === 'Relationship (Love)') {
      prevSection -= 1;
    } else if (formData.spousePermitStatus === 'work' && sections[prevSection].title === 'On Study Permit') {
      prevSection -= 1;
    } else if (formData.spousePermitStatus === 'study' && sections[prevSection].title === 'On Work Permit') {
      prevSection -= 1;
    }
        ...
        <div className="p-4 lg:pl-[60px] w-full lg:w-[95%] py-12 lg:pb-16 lg:pr-0 sm:p-12">
          ...
          <div className="flex justify-center">
            <div className="flex flex-row justify-between w-full mt-10">
              <button
                className={`step4 bg-black text-white px-4 py-3.5 min-w-[100px] text-sm font-bold${currentSection === 0 ? ' opacity-50 cursor-not-allowed' : ''}`}
                type="button"
